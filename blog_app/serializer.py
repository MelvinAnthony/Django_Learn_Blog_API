...
class BlogSerializer(serializers.ModelSerializer):
    ...
    def get_comments(self, obj):
        comments = BlogComment.objects.filter(blog=obj)[:3]  # Limiting to 3 comments
        request = self.context.get('request')
        return {
            "comments": BlogCommentSerializer(comments, many=True).data,
            "all_comment_link": request.build_absolute_uri(reverse('blog_commant_list', kwargs={'blog_id':obj.id}))
        }
